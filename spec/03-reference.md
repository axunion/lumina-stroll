...
| ミュートランプ | `setTargetAtTime(0, now, 0.05)` | ミュート切替時のゲインランプ（クリックノイズ防止）。ミュート解除も同じ時定数でマスターゲインへランプする |
...
| Cave 雫 | sine の短いプラック（880 → 440 Hz を 0.3 秒で exp ランプ、attack 5 ms） | 発生間隔 7–15 秒の一様乱数 / ゲイン 0.08 |
...
| 碑文発見ベル | triangle | 528 Hz、attack 5 ms、減衰 2 秒、ゲイン 0.12 |
